...
class Clock:
    ...
    def main(self, cathode_poison_method, settings):
        ...
        while True:
            ...
            hour, minute, sec = self.basic_time(seconds=seconds_display,
                                                twenty_four=twenty_four_hour,
                                                display_time=display_time,
                                                offset=offset)

            self.poisoning_manager(poison_func=anti_poison_func, hour=hour, sec=sec, minute=minute)
    ...
    def basic_time(self, seconds=True, twenty_four=True, display_time=True, offset='+0'):
        ...
        return actual_hour, minute, actual_sec

    def poisoning_manager(self, poison_func, hour, minute, sec):
        if hour != self.last_hour:
            self.anti_poisoning = False
            self.last_hour = hour
        if ((minute * 60 + sec) >= self.rand_sec) and not self.anti_poisoning:
            poison_func()
